...
KNN_MODEL_FILE = os.path.join(MODEL_PATH, "knn_model ardra.pkl")
BERT_MODEL_FILE = os.path.join(MODEL_PATH, "bert_model ardra.pkl")
EMBEDDINGS_FILE = os.path.join(MODEL_PATH, "job_embeddings ardra.npy")
DATA_FILE = os.path.join(MODEL_PATH, "preprocessed_data ardra.csv")


def preprocess_text(df):
    """Preprocess job descriptions by combining relevant columns."""
    df = df.copy()
...
def expand_skills(skills):
    """Expand skills input using WordNet synonyms."""
...
def build_and_save_model(df):
    """Builds and saves BERT and KNN models using pickle."""
    os.makedirs(MODEL_PATH, exist_ok=True)

    print("\n\033[1;36mBuilding model... Please wait.\033[0m")
    ...
    print("\033[1;32mModel trained and saved successfully!\033[0m")
    return model, knn, job_embeddings


def load_model():
    """Loads saved model components from disk."""
    if all(os.path.exists(f) for f in [KNN_MODEL_FILE, BERT_MODEL_FILE, EMBEDDINGS_FILE, DATA_FILE]):
        print("\033[1;34mLoading saved model...\033[0m")

        with open(BERT_MODEL_FILE, 'rb') as f:
            model = pickle.load(f)
...
if __name__ == "__main__":
    try:
        model, knn, job_embeddings, df = load_model()

        if model is None:
            print("\033[1;33mNo saved model found. Training from scratch...\033[0m")
            file_path = r"C:\\Users\\DELL\\Downloads\\Jobs - Jobs.csv"
            df = pd.read_csv(file_path)
            df = preprocess_text(df)
            model, knn, job_embeddings = build_and_save_model(df)
        else:
            print("\033[1;32mModel loaded. Ready to predict!\033[0m")

        user_input = input("\nEnter your skills: ")
        recommendations, mean_similarity, median_similarity = get_recommendations(
            user_input, df, model, knn, job_embeddings
        )
